...
pub fn restrict_resources(hostname: &String, pid: Pid) -> Result<()> {
    ...
    let pid: u64 = pid.as_raw().try_into()?;
    cgs.add_task(CgroupPid::from(pid))?;
    setrlimit(Resource::NOFILE, NOFILE_RLIMIT, NOFILE_RLIMIT)?;
    Ok(())
}

pub fn clean_cgroups(hostname: &String) -> Result<()> {
    debug!("Cleaning cgroups");
    let path = canonicalize(format!("/sys/fs/cgroup/{}/", hostname))?;
    remove_dir(&path)?;
    Ok(())
}
